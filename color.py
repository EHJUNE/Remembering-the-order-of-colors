...
change = "sound/screenchange.mp3"
correct = "sound/correct.mp3"
incorrect = "sound/incorrect.mp3"
...
levels = [
    ...
    (8, [0, 1, 2, 3, 4]),       # 난 4: 8 라운드, 빨, 파, 초, 노, 보
    (10, [0, 1, 2, 3, 4, 5]),
    (12, [0, 1, 2, 3, 4, 5, 6])
]
...
# 게임 오버 화면 표시
def game_over_screen(sequence):
    screen.fill((0, 0, 0))                      # 화면 검은색
    display_text("GAME OVER", y_offset=-20)     # 게임 오버 텍스트 생성
    text = 'Correct : '
    for i in range(len(sequence)):              #실패했을 때 정답 보여주기
        if sequence[i] == 0:
            text = text + '1'
        elif sequence[i] == 1:
            text = text + '2'
    ...
    display_text(str(text), y_offset=20)
    pygame.display.flip()                       # 화면 전환
    time.sleep(2)                               # 2초 대기
...
def main_game():
    ...
    while level < len(levels):
        ...
        pygame.mixer.music.load(change)
        ...
        if time.time() - start_time >= input_timeout:
            ...
            game_over_screen(sequence)      # 게임 오버 화면 전환
            return "game_over"
        ...
        if user_sequence == sequence:
            ...
            game_over_screen(sequence)      # 게임 오버 화면 표시
            return "game_over"
